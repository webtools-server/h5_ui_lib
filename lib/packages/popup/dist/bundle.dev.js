...
/*``*/) {
var out='<div class="'+(it.classes.wrap)+'"> <div class="ui-overlay '+(it.classes.mask)+'"></div> <div class="ui-popup"> <div class="ui-popup__title '+(it.classes.title)+'">';if(it.title){out+=' '+(it.title)+' ';}out+=' ';if(it.showClose){out+=' <div et="click:close" class="ui-popup__fn-button"><i class="'+(it.classes.close)+'"></i></div> ';}out+=' </div> <div class="ui-popup__content '+(it.classes.content)+'">'+(it.content)+'</div> ';if(it.footer){out+=' <div class="ui-popup__footer '+(it.classes.footer)+'">'+(it.footer)+'</div> ';}out+=' </div></div>';return out;
}
...
function calcContentHeight(topHeight = 296, titleHeight = 104) {
  return `${window.innerHeight - (topHeight + titleHeight) / 2}px`;
}
...
class Popup {
  ...
  _bindEvent() {
    this.$el.on('click', e => {
      ...
      tag !== 'A' && e.stopPropagation(); // 是对应的事件
  ...
  hide() {
    ...
    this.$el.find(selectors.el).removeClass(modifyClasses.popupShow).one('webkitTransitionEnd', () => {
      this.$el.remove();
